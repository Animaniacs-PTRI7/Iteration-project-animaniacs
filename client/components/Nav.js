...
export default function Nav(props) {
  ...
  if (props.logOut) {
    <div>
      ...
      >
        <IconButton onClick={props.logOut}></IconButton>
      </Tooltip>
      <Tooltip
        title={<h2 style={{ color: "white" }}>My Orders</h2>}
        TransitionComponent={Zoom}
      >
        <IconButton component={Link} to="/MyOrders"></IconButton>
      </Tooltip>
    </div>;
        ...
        <Toolbar className={classes.appbarWrap}>
          <Link to="/" className={classes.noUnderline}>
            <h1 className={classes.appbarHead}>
              <span data-testid="nav-1" className={classes.logoBlack}>
                Only
              </span>
              <span className={classes.logoRed}>Pans</span>
              <DiningIcon />
            </h1>
          </Link>
          <div></div>
